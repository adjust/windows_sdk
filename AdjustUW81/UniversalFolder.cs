...
{
    /// <summary>
    /// Represents a folder in the <see cref="UniversalFileSystem"/>
    /// </summary>
    public class UniversalFolder : IFolder
    {
        private readonly IStorageFolder _wrappedFolder;
        private readonly bool _isRootFolder;

        /// <summary>
        /// Creates a new <see cref="UniversalFolder"/>
        /// </summary>
        /// <param name="wrappedFolder">The WinRT <see cref="IStorageFolder"/> to wrap</param>
        public UniversalFolder(IStorageFolder wrappedFolder)
        {
        ...
        public async Task<IFile> CreateFileAsync(string desiredName, PCLStorage.CreationCollisionOption option, CancellationToken cancellationToken)
        {
            ...
            {
                if (ex.HResult == UniversalFile.FILE_ALREADY_EXISTS)
                {
            ...
            return new UniversalFile(wrtFile);
        }
        ...
        public async Task<IList<IFile>> GetFilesAsync(CancellationToken cancellationToken)
        {
            ...
            var files = wrtFiles.Select(f => new UniversalFile(f)).ToList<IFile>();
        ...
        public async Task<IFolder> CreateFolderAsync(string desiredName, PCLStorage.CreationCollisionOption option, CancellationToken cancellationToken)
        {
            ...
            await EnsureExistsAsync(cancellationToken).ConfigureAwait(false);
            StorageFolder universalFolder;
            try
            {
                universalFolder = await _wrappedFolder.CreateFolderAsync(desiredName, GetWinRTCreationCollisionOption(option)).AsTask(cancellationToken).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                if (ex.HResult == UniversalFile.FILE_ALREADY_EXISTS)
            ...
            return new UniversalFolder(universalFolder);
        }
        ...
        public async Task<IFolder> GetFolderAsync(string name, CancellationToken cancellationToken)
        {
            ...
            try
            {
                universalFolder = await _wrappedFolder.GetFolderAsync(name).AsTask(cancellationToken).ConfigureAwait(false);
            }
            catch (FileNotFoundException ex)
            {
        ...
        public async Task<IList<IFolder>> GetFoldersAsync(CancellationToken cancellationToken)
        {
            ...
            var folders = wrtFolders.Select(f => new UniversalFolder(f)).ToList<IFolder>();
            return new ReadOnlyCollection<IFolder>(folders);
        }

        internal static Task<Task<T>> AsTaskNoThrow<T>(IAsyncOperation<T> operation, CancellationToken cancellationToken)
        {
        ...
        public async Task<ExistenceCheckResult?> CheckExistsAsync(string name, CancellationToken cancellationToken)
        {
            ...
            var itemAsync = _wrappedFolder.GetItemAsync(name);
            var result = await AsTaskNoThrow(itemAsync, cancellationToken);
